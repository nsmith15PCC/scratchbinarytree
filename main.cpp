...
void perform(bst<char> tree[], char selection, int &touse)
{
   ...
   switch(toupper(selection))
   {
     ...
     case 'C' : cout<<"There are "<<tree[touse].size()<<" items in the tree "<<endl;
                break;

     case 'N' : cout<<"There were "<<tree[touse].size()<<" items in the tree you just disposed"<<endl;
                tree[touse].clear();
                break;
     ...
     case 'S' : do
                ...
                switch(order)
                {
                    case 1: tree[touse].traversal() = PREORDER;
                            break;
                    case 2: tree[touse].traversal() = INORDER;
                            break;
                    case 3: tree[touse].traversal() = POSTORDER;
                            break;
                }
                break;
